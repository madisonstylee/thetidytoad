...
async function checkAuth() {
  try {
    ...
    // Return the user credential for use in other checks (don’t delete yet)
    ...
    throw error; // Propagate the error to stop execution
  }
}

// Check Firestore Database (requires authenticated user)
async function checkFirestore(userCredential) {
  try {
    ...
    // Write to Firestore (authenticated request)
    ...
    if (error.code === 'permission-denied') {
      ...
      console.error('❌ Make sure the user is authenticated before accessing Firestore');
    }
...
async function runChecks() {
  ...
  try {
    // Step 1: Check Auth and get user credential
